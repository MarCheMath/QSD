...
#     return ["Person A"] + assignments[list(assignments.keys())[0]]["Person B"]
...
#@pytest.fixture(name='assignments')
def get_assignments(names):
    cm = CircleMethod(names)
    cm.compute_table()
    print_CMRT(cm)
    return cm.roundTable

def test_check_table(assignments,names):
...
test_check_table(get_assignments(get_names()), get_names())